// Package flags is the bike shop's feature-flag client — OpenFeature (ADR-099) evaluated in-process by the
...
// flag decision shows up directly in the checkout trace. Fail-static: if flagship is unreachable the resolver
// keeps the last-known set (or serves in-code defaults on cold start), never blocking a checkout.
package flags
...
// evaluations return their in-code defaults. Non-blocking: the shop starts even if flagship is down (evals
...
func Setup(ctx context.Context, syncURL string) (*openfeature.Client, error) {
	...
	if syncURL == "" {
		return openfeature.NewClient("alpha-shop"), nil
	}
...
// EvalContext builds an evaluation context keyed on the shop session id — flagship's `fractional` rollout ops
// bucket on this targetingKey, so a percentage rollout is sticky per visitor.
func EvalContext(sessionID string) openfeature.EvaluationContext {
	return openfeature.NewEvaluationContext(sessionID, map[string]any{})
}
